...
public class MultiThreadRequestHandler extends RequestHandler {
	...
	public MultiThreadRequestHandler(BusinessLogicFactory businessLogicFactory, Socket socket) {
	...
	protected void processRequestWithoutData(Request request) {
		...
		DataFileReader fileReader = new DataFileReader(request.getInpath(), outQueue, isCurrentDone);
		Thread readerThread = new Thread(fileReader);
		readerThread.start();

		List<String> businessLogics = conf.getBusinessLogics();
		List<String> businessLogicConfigurationString = conf.getBusinessLogicConfigurationStrings();
		...
		for (int i = 0; i < businessLogics.size(); i++) {
			...
			BusinessLogic businessLogicInstance = businessLogicFactory.getBusinessLogicInstance(businessLogics.get(i),
					businessLogicConfigurationString.get(i), null, inQueue, outQueue, isPreviousDone, isCurrentDone);
			businessLogicThread = new Thread(businessLogicInstance);
			businessLogicThread.start();
		}

		DataFileWriter fileWriter = new DataFileWriter(request.getOutpath(), outQueue, isCurrentDone);
		fileReader.run();
	}
}